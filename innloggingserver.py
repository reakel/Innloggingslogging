...
#Dette skriptet kjører kontinuerlig, og tar i mot innloggingsmeldinger fra salmaskiner. Disse parses og loggføres i databasen.
...
#	try:
	...
	if  id == 'login':
		...
		with dbcon:
			...
			isotid=inntid[6:10]+'-'+inntid[3:5]+'-'+inntid[:2]+' '+inntid[11:]
			if sistinst!='ukjent':
				isoinsttid=sistinst[6:10]+'-'+sistinst[3:5]+'-'+sistinst[:2]
			else:	
				isoinsttid='ukjent'
...
#		print info
		...
		with dbcon:
			...
			try:
				if (isotid[-5:]!="03:01"): #litt haxx for kanskje aa utelukke noen feil
#					print "utlogg: "+mnavn+" "+isotid
					...
					#test om denne innloggingen allerede finnes i sessions - hvis ja mangler en innlogging, og sessionen må droppes(?)
